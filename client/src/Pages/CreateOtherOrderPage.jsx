...
export default function CreateOtherOrderPage() {
    ...
    const [fetch, loading, error] = useFetching(async (type) => {
        switch (type) {
            case "create":
                {
                    if (customer) {
                        const res = await OrderService.Create(newOrderData, customer.id)
                        navigate(-1)
                    }
                }
                break;
    ...
    const onCustomerChange = (newClientData) => {
        setCustomer(newClientData)
    }
    ...
    return (
        <div className="container-fluid mt-5 p-5">
            <div className='mt-5 align-items-stretch d-flex flex-column'>
                ...
                <button className='btn btn-primary display-7 rounded-pill'
                    data-bs-toggle="modal" data-bs-target="#clientId">
                    {customer ? `Клиент №${customer.id}; ${customer.name}` : 'Выбрать клиента'}
                </button>
